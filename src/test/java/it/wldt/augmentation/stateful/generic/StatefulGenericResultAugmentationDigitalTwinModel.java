...
public class StatefulGenericResultAugmentationDigitalTwinModel extends DigitalTwinModel {

    private static final WldtLogger logger = WldtLoggerProvider.getLogger(StatefulGenericResultAugmentationDigitalTwinModel.class);

    private boolean isShadowed = false;

    public StatefulGenericResultAugmentationDigitalTwinModel() {
    ...
    protected void onAugmentationFunctionListAvailable(String handlerId, List<AugmentationFunction> augmentationFunctionList) {
        try {
            ...
            for(AugmentationFunction augmentationFunction : augmentationFunctionList) {
                ...
                // Save the metric to evaluate received notifications
                SharedTestMetrics.getInstance().addAugmentationFunctionRegistrationCallback(this.digitalTwinStateManager.getDigitalTwinId(), handlerId, augmentationFunction);

                // If the Augmentation Function correspond to the target type and it is stateful
