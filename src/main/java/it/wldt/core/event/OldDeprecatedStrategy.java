 ...
 *
 * Original synchronous implementation of WldtEventBusStrategy — now deprecated.
 *
 * This was the original default strategy of the WLDT event bus. It serializes all
 * operations on a single instance-level lock and calls onEvent() in the publisher's
 * thread while the lock is held.
 *
 * Retained for backward compatibility and testing. For new deployments, use
 * {@link DefaultEventBusStrategy}, which provides publisher isolation and per-subscriber
 * thread isolation without blocking the caller.
 *
 * @deprecated Use {@link DefaultEventBusStrategy} instead.
 */
@Deprecated
public class OldDeprecatedStrategy implements WldtEventBusStrategy {

    private static final WldtLogger logger = WldtLoggerProvider.getLogger(OldDeprecatedStrategy.class);
