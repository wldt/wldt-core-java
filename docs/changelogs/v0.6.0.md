...
### Refactoring for Content-Type in PhysicalAssetProperty

Added `contentType` (String) field to the `PhysicalAssetProperty` class to allow adapters to communicate
the content type of the property value (e.g. `application/json`), mirroring the same mechanism introduced for `PhysicalAssetEvent`.
The field is fully accessible via getter and setter methods.

The default 2-argument constructor `PhysicalAssetProperty(key, initialValue)` now sets `type` equal to the `key` and
derives `contentType` automatically from the initial value's Class (e.g. `java.lang.Double`).
New constructors have been added to let the developer set these fields explicitly:

- `PhysicalAssetProperty(key, type, initialValue)`: sets a custom `type`, deriving `contentType` from the initial value's Class (logs a warning and sets `contentType` to `null` if `initialValue` is `null`).
- `PhysicalAssetProperty(key, type, contentType, initialValue)`: sets `type` and `contentType` explicitly (defaults `immutable=false`, `writable=true`).
- `PhysicalAssetProperty(key, type, contentType, initialValue, immutable, writable)`: full constructor exposing every field.

With this change the `PhysicalAssetProperty` now includes the following fields:

- `key`: Key uniquely identifying the property in the Digital Twin State.
- `type`: Domain specific type of the property (e.g., "iot.demo.temperature") that the developer can define, defaulting to the property `key` if not specified.
- `contentType`: The content type of the property value (e.g., "application/json") that allows the receiving Digital Twin Model to correctly interpret the value representation, derived by default from the value's Class.
- `initialValue`: Initial value of the property from the Physical Twin.
- `immutable`: Identifies whether the property is immutable by external modules.
- `writable`: Identifies whether the property is writable by external modules.
...
- `key`: Key uniquely identifying the Physical Event on the Physical Twin.
